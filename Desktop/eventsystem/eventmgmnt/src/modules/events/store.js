...
export default {
  ...
  actions: {
    ...
    getEvent: (context, no) => {
      ...
      call("get", constants.events(no))
        .then((res) => {
          context.commit("SET_EVENT", res.data.data.shift());
          context.commit("SET_LOADING", false, { root: true });
        })
        .catch((error) => {
          Event.$emit("ApiError", error.response.data.message);
    ...
    searchProfile: (context, term) => {
      context.commit("SET_LOADING_SEARCH", true);
      call("get", constants.profile(term))
        .then((res) => {
          if (!res.data.data[0].MemberNo) {
            Event.$emit(
              "ApiError",
              "Members number does not exist. Please input member number in the Format M.0000 or G.0000"
            );
            context.commit("SET_LOADING_SEARCH", false);
            return;
          }
          Event.$emit("packageLoaded", true);
          context.commit("SET_PROFILE", res.data.data[0]);
          context.commit("SET_LOADING_SEARCH", false);
        })
        .catch((error) => {
          Event.$emit("packageLoaded", false);
          Event.$emit("ApiError", error.response.data.message);
          context.commit("SET_LOADING_SEARCH", false);
        });
    },
    ...
    updateBookingCurrency(context, payload) {
      call("post", constants.updateBookingCurrency, payload)
        .then((res) => {
          console.assert(res);
        })
        .catch((error) => {
          ...
          Event.$emit(
            "ApiError",
            "Sorry, We ran into an error updating booking currency."
          );
          context.commit("SET_VALIDATING", false);
        });
    },
